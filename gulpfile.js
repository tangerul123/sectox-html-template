...
function copy(folder, done) {
    gulp.src(`./src/assets/${folder}/**/*`).pipe(
        gulp.dest(`./dest/assets/${folder}`)
    )
    done()
}
...
gulp.task("images", function (done) {
    ...
    gulp.src(imageSrc).pipe(gulp.dest(dest.images))
    done()
})
...
function watchFiles() {
    ...
    gulp.watch(src.scss, gulp.series(["style", "html"], reload))
}
